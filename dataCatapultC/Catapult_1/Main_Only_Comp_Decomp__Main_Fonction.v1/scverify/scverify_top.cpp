...
void scverify_top::setup_debug()
{
   ...
   #ifdef MC_DEFAULT_TRANSACTOR_LOG
      static int Main_Only_Comp_Decomp__Main_Fonction_return_flags = MC_DEFAULT_TRANSACTOR_LOG;
   #else
      static int Main_Only_Comp_Decomp__Main_Fonction_return_flags = MC_TRANSACTOR_UNDERFLOW | MC_TRANSACTOR_WAIT;
   #endif
      static int Main_Only_Comp_Decomp__Main_Fonction_return_count = -1;
      ...
      debug("Src:317:Src",Src_flags,Src_count);
      debug("Comp:319:Comp",Comp_flags,Comp_count);
      debug("Vga:320:Vga",Vga_flags,Vga_count);
      debug("nblevels:321:nblevels",nblevels_flags,nblevels_count);
      debug("return:322:return",Main_Only_Comp_Decomp__Main_Fonction_return_flags,Main_Only_Comp_Decomp__Main_Fonction_return_count);
}
// ============================================
...
void scverify_top::install_observe_foreign_signals()
{
   ...
   #ifdef MTI_SYSTEMC
     OFS_Main_Only_Comp_Decomp_Main_Fonction_core_inst_core_wen.observe_foreign_signal("/scverify_top/rtl/Main_Only_Comp_Decomp_Main_Fonction_core_inst/core_wen");
   ...
   #endif
   #endif
     HDL_CONNECT_FN(OFS_Main_Only_Comp_Decomp_Main_Fonction_core_inst_core_wen,"scverify_top.rtl.Main_Only_Comp_Decomp_Main_Fonction_core_inst.core_wen",HDL_INPUT,0);
...
void scverify_top::debug(const char *varname, int flags, int count)
{
   ...
   if (strcmp(varname,"Src:317:Src") == 0) {
      xlator_p = Src_transactor;
   }
   if (strcmp(varname,"Comp:319:Comp") == 0) {
      xlator_p = Comp_transactor;
   }
   if (strcmp(varname,"Vga:320:Vga") == 0) {
      xlator_p = Vga_transactor;
   }
   if (strcmp(varname,"nblevels:321:nblevels") == 0) {
   }
   if (strcmp(varname,"return:322:return") == 0) {
      xlator_p = Main_Only_Comp_Decomp__Main_Fonction_return_transactor;
   }
   if (xlator_p) {
...
// Static sensitivity: sensitive  << OFS_Main_Only_Comp_Decomp_Main_Fonction_core_inst_core_wen << clk;
...
void scverify_top::deadlock_watch() {
   // DEADLOCK_WATCH
      ...
      {
         if (
            (OFS_Main_Only_Comp_Decomp_Main_Fonction_core_inst_core_wen.read() == SC_LOGIC_0) &&
            (OFS_mgc_start_sync_mgc_bsync_vld_vd.read() == SC_LOGIC_1)
         )
         {
            deadlocked.write(SC_LOGIC_1);
            deadlock_event.notify(201959168.28234, SC_NS);
         } else {
            if (deadlocked.read() == SC_LOGIC_1) {
...
void scverify_top::generate_reset() {
      ...
      if (first || sc_time_stamp() == SC_ZERO_TIME)
      {
         ...
         Main_Only_Comp_Decomp__Main_Fonction_return_transactor->reset_streams();
